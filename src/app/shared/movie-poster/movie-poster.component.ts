...
export class MoviePosterComponent implements OnInit {
  ...
  rateFilm(target) {
    if (!this.moviePoster.results[0].login_user_checked.rating || target.value !==
      this.moviePoster.results[0].login_user_checked.rating  ) {
      this.moviePoster.results[0].login_user_checked.rating = target.value;

      // 평가가 된적이 없다는 것이므로 평가한 별점을 db에 보낸다.
      // 없엇던 리소스를 추가하는 것이기 때문에 put을 통해 요청한다.
    } else if (target.value === this.moviePoster.results[0].login_user_checked.rating) {

      // 평가한 별점이 이미 평가한 별점과 같다는 것으로 평가를 취소한다. === db의 평가를 false로 만든다.
      // 있던 rate를 수정하는 것이기 때문에 patch를 통해 요청한다.
      this.moviePoster.results[0].login_user_checked.rating = 0;
    }
  }
  ngOnInit() {}
}
